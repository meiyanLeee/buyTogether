...
// request拦截器，将用户token放入头中（每次请求动态读取）
axios.interceptors.request.use(
  config => {
    let token = getAuthToken();
    if(token) {
      config.headers['satoken'] = token;
      console.log('[Auth] 携带token请求:', config.url, 'token前8位:', token.substring(0, 8));
    } else {
      console.log('[Auth] 无token请求:', config.url);
    }
    return config
  },
  error => {
...
axios.interceptors.response.use(function (response) {
  ...
  return Promise.reject("服务暂时繁忙，请稍后再试");
});
